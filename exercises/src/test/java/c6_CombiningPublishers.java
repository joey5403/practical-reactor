...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void behold_flatmap() {
        ...
        Mono<String> currentUserEmail = getUserEmail(currentUserMono.block());
    ...
    public void i_am_rubber_you_are_glue() {
        ...
        numberService1().concatWith(
        numberService2());
        numberService1().mergeWith(
                numberService2());
    ...
    public void task_executor_again() {
        //todo: feel free to change code as you need
        Flux<Void> tasks =
        taskExecutor().flatMapSequential(Function.identity());
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail =
                mailBoxPrimary().switchOnFirst((first, primary) -> {
                            if (Objects.requireNonNull(first.get()).metaData.contains("spam")) {
    ...
    public void acid_durability() {
        ...
        Flux<String> committedTasksIds =
                tasksToExecute().flatMapSequential(task -> {
                    commitTask(task.block());
                    return task;
                });
